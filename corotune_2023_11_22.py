...
class ex:
    def _2():
        def f(x, y, z, w): return ((not(x) or y) or (not(z) or w)) and not (not(x) or w)
        s = [0,1]
        print('x w z y >')
        for _ in s:
            for x in s:
    ...
    def _49():
        ...
        for i, v in enumerate( range(1371085, 1371134+1) ):
            allow = True
            for sqr in s(v, False):
                allow = False
                break
            if allow: print(i+1, v, lst)
    ...
    def _151():
        ...
        def count(v):
            lst = set()
            for sqr in s(v, True):
                if sqr[0]%2: lst.add(sqr[0])
                if sqr[1]%2: lst.add(sqr[1])
                if len(lst) > 5: break
            if len(lst) == 5: print(v, max(lst))

        # нечетные делители - сами нечетные числа
        for n in range(78_000_000, 85_000_000+1):
            while n%2 == 0: n //= 2
            count(n)
